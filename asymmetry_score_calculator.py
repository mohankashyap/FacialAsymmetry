"""
Created on Thu May 30 14:05:20 2024
...
from misc import mapperLandmarksToCordinatesEDMA, mapperLandmarksToCordinatesMirrorError,mouth_ul_ll_to_rm_lm
...
def symmetryEDMA(landmarks,frames):
    ...
    dict_outcomes_edma = {
        "Eye(edma)": np.mean(list(eye_region_scores.values())),
        "Eyebrow(edma)": np.mean(list(eyebrow_region_scores.values())),
        "Mouth(edma)": np.mean(list(mouth_region_scores.values())),
        "Overall(edma)": np.mean(list(eye_region_scores.values())) + np.mean(
...
def symmetryMirrorError(Landmarks,frames):
    ...
    dict_outcomes_mirror_error = {
        "Eye(mirror_error)": np.mean(list(mirror_error_eye_brow.values())),
        "Eyebrow(mirror_error)": np.mean(list(mirror_error_eye_region.values())),
        "Mouth(mirror_error)": np.mean(list(mirror_error_mouth_region.values())),
        "overall(mirror_error)": np.mean(list(mirror_error_overall.values()))
    }
